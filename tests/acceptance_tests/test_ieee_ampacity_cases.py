...
@pytest.fixture
def example_span_C(drake_conductor_B: linerate.Conductor) -> linerate.Span:
    start_tower = linerate.Tower(latitude=43, longitude=0, altitude=0)
    end_tower = linerate.Tower(latitude=43, longitude=0, altitude=0)
    return linerate.Span(
        conductor=drake_conductor_B,
...
@pytest.fixture
def example_model_C(
    example_span_C: linerate.Span, example_weather_B: linerate.Weather
) -> linerate.IEEE738:
    return linerate.IEEE738(example_span_C, example_weather_B, np.datetime64("2016-06-10 14:00"))


def test_example_C_solar_heating(example_model_C: linerate.ThermalModel):
    assert example_model_C.compute_solar_heating() == approx(13.732, abs=0.5)


def test_example_C_convective_cooling(example_model_C: linerate.ThermalModel):
